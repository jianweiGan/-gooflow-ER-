var App = function() {
	...
	var handleSidebar = function() {
		...
		jQuery('.sidebar-menu .has-sub > a').click(function() {
         ...
         //这是点击第二级目录
		// Handle sub-sub menus： has-sub第一级  sub第二级  has-sub-sub第三级，sub-sub第四级
	...
	return {
		...
		init: function() {
			...
			if(App.isPage("DataAuditManagement")) {

			}
			if(App.isPage("UserRoleManagement")) {
				$("#userRoleMag").toggleClass("open");
			}
			if(App.isPage("OperationStatistics")) {
				//handleUniform();		//Function to display 
			}
			checkLayout(); //Function to check if mini menu/fixed header is activated
			handleSidebar(); //显示侧边栏
			handleSidebarCollapse(); //隐藏或显示侧边栏
