var isStartNodeExisted = false; //起始节点只能有一个
var endCnt = 0;
var databaseCnt = 0;
var dbtableCnt = 0;
var functionCnt = 0;
...
function GooFlow(bgDiv, property) {
	...
	this.$title = property.headLabel; //流程图的名称
	...
	if(property.haveHead) {
		...
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='新建' title='新建'><i class='ico_" + property.headBtns[0] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='打开' title='打开'><i class='ico_" + property.headBtns[1] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='保存' title='保存'><i class='ico_" + property.headBtns[2] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='回撤' title='回撤'><i class='ico_" + property.headBtns[3] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='回复' title='恢复'><i class='ico_" + property.headBtns[4] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='刷新' title='刷新'><i class='ico_" + property.headBtns[5] + "'></i></a>"
		tmp += "<a href='javascript:void(0)'class='GooFlow_head_btn' alt='执行' title='执行'><i class='ico_" + property.headBtns[6] + "'></i></a>"
		tmp += "</div>";
		this.$head = $(tmp);
		...
		this.onBtnStartClick = null; //执行流程图按钮定义
	...
	if(property.haveTool) {
		this.$bgDiv.append("<div class='GooFlow_tool'" + (property.haveHead ? "" : " style='margin-top:2px'") + "><div style='height:" + (height - headHeight - (property.haveHead ? 7 : 10)) + "px' class='GooFlow_tool_div'></div></div>");
		...
		if(property.toolBtns && property.toolBtns.length > 0) {
			...
			for(var i = 0; i < property.toolBtns.length; ++i) {
				var splitLine = property.toolBtns[i].indexOf("ellipse");
				var splitLine2 = property.toolBtns[i].indexOf("diamond");
		...
		toolWidth = $(".GooFlow_tool_div").width();
		this.$nowType = "cursor";
		//绑定各个按钮的点击事件
		this.$tool.on("click", {
			inthis: this
	...
	width = width - toolWidth - 15; //工作区的宽度要减去工具条的宽度，否则显示不出来
	...
	this.$workArea = $("<div class='GooFlow_work_inner' style='overflow:yes;width:" + width * 1 + "px;height:" + height * 1 + "px'></div>")
		.attr({
			"unselectable": "on",
	...
	if(this.$editable) {
		this.$workArea.on("click", {
			inthis: this
		}, function(e) { //注册点击工作区添加节点，连线，区域的事件
			if(!e) e = window.event;
			var This = e.data.inthis;
		...
		if(property.useOperStack && this.$editable) { //如果要使用堆栈记录操作并提供“撤销/重做”的功能,只在编辑状态下有效
			...
			this.onBtnStartClick = function() {
				jsonAction = JSON.stringify(container.exportData());
				jsonAction.toString();
				var url = "http://localhost:8080/fujian-demo/service/DataTransferService";
				var p = new SOAPClientParameters();
				var currentTime = getNowFormatDate().toString();
				p.add("jsonArray", jsonAction);
				p.add("sendRequestTime", currentTime);
				SOAPClient.explicitNS = true;
				SOAPClient.invoke(url, "execDataTransferRecord", p, true, ExecWorkFlow_CallBack);
			}
			//打开现有流程
			this.onBtnOpenClick = function() {
				//清理屏幕
				if(confirm("是否保存当前作业流程？")) {
					jsonAction = JSON.stringify(container.exportData());
					jsonAction.toString();
					var url = "http://localhost:8080/fujian-demo/service/DataTransferService";
					var p = new SOAPClientParameters();
					...
					p.add("sendRequestTime", currentTime);
					SOAPClient.explicitNS = true;
					SOAPClient.invoke(url, "storeDataTransferRecord", p, false, HelloTo_callBack);

					function HelloTo_callBack(r) {
						var currentTime = getNowFormatDate();
						currentTime = Date.parse(currentTime);
						var sendRequestTime = r.result.sendRequestTime;
						sendRequestTime = Date.parse(sendRequestTime);
						var spendTime = (currentTime - sendRequestTime) / 1000; //用时
						var storeStatusInfo = r.result.storeStatusInfo;
						alert(storeStatusInfo + "总用时:" + spendTime + "秒");
					}
				} else {
				...
				container.clearData();
				isStartNodeExisted = false;
				container.loadData(jsondata);
			};
			//保存本次流程图到服务器
			this.onBtnSaveClick = function() {
				...
				p.add("sendRequestTime", currentTime);
				SOAPClient.explicitNS = true;
				SOAPClient.invoke(url, "storeDataTransferRecord", p, true, HelloTo_callBack);
...
GooFlow.prototype = {
	...
	initDraw: function(id, width, height) {
		...
		$(this.$draw).delegate(tmpClk, "dblclick", {
			...
			This.$textArea.val(oldTxt).css({
				...
				width: 120,
				height: 20,
				left: t.left + x - This.$workArea[0].parentNode.scrollLeft - 250,
	...
	//增加一个流程结点,传参为一个JSON,有id,name,top,left,width,height,type(结点类型)等属性，top和left用来确定node位置
	addNode: function(id, json, isLoadJsonData) {
		if(isStartNodeExisted) {
			var para1 = /start/;
			if(para1.test(json.type)) {
				alert("一个作业中只能包含一个作业起点！");
				return;
			}
		}
		var para1 = /start/;
		var originalJsonName = json.name;
		if(para1.test(json.type)) {
			isStartNodeExisted = true;
			json.name = "作业起点";
		}
		var nodeType = json.type;
		if(nodeType == "end round") {
			json.name = "作业终点" + ++endCnt;
		}
		if(nodeType == "database rect") {
			json.name = "数据库" + ++databaseCnt;
		}
		if(nodeType == "dbtable rect") {
			json.name = "数据库表" + ++dbtableCnt;
		}
		if(nodeType == "_function rect") {
			json.name = "函数" + ++functionCnt;
		}
		if(nodeType == "event rect") {
			json.name = "事件" + ++eventCnt;
		}
		if(nodeType == "query rect") {
			json.name = "查询" + ++queryCnt;
		}
		if(nodeType == "report rect") {
			json.name = "报表" + ++reportCnt;
		}
		if(nodeType == "backup rect") {
			json.name = "备份" + ++backupCnt;
		}
		if(nodeType == "field ellipse") {
			json.name = "字段" + ++fieldCnt;
		}
		if(nodeType == "operate diamond") {
			json.name = "操作" + ++relationCnt;
		}
		//如果是载入已有的流程图，则应该把nodeName设为json本身的名字。
		if(isLoadJsonData) {
			json.name = originalJsonName;
		}
		...
		var mark = json.marked ? " item_mark" : ""; //是否高亮显示
		if(json.type.indexOf("rect") > 0) { //矩形，表示实体
			if(!json.width || json.width < 100) json.width = 120;
			if(!json.height || json.height < 24) json.height = 32;
			if(!json.top || json.top < 0) json.top = 0;
			if(!json.left || json.left < 0) json.left = 0;
			var hack = 0;
			if(navigator.userAgent.indexOf("8.0") != -1) hack = 2;
			this.$nodeDom[id] = $("<div class='GooFlow_item item_rect" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px' nodeInfo=" + json.nodeInfo + "><table cellspacing='1' style='width:" + (json.width - 2) + "px;height:" + (json.height - 2) + "px;'><tr><td class='ico'><i class='ico_" + json.type + "'></i></td><td>" + json.name + "</td></tr></table><div style='display:none'><div class='rs_bottom'></div><div class='rs_right'></div><div class='rs_rb'></div><div class='rs_close'></div></div></div>");
		} else if(json.type.indexOf("round") > 0) { //圆形 表示起点和终点
			json.width = 32;
			json.height = 32;
			this.$nodeDom[id] = $("<div class='GooFlow_item item_round" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px' nodeInfo=" + json.nodeInfo + "><table cellspacing='0'><tr><td class='ico'><i class='ico_" + json.type + "'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>" + json.name + "</div></div>");
		} else if(json.type.indexOf("diamond") > 0) { //菱形 表示联系,,原始ico的png要右移一定距离,使用offset_ico图标
			json.width = 120;
			json.height = 32;
			this.$nodeDom[id] = $("<div class='GooFlow_item item_diamond" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px' nodeInfo=" + json.nodeInfo + "><table cellspacing='0'><tr><td class='ico'><i class='offset_ico_" + json.type + "'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>" + json.name + "</div></div>");
		} else if(json.type.indexOf("ellipse") > 0) { //椭圆形 表示表格字段属性,原始ico的png要右移一定距离,使用offset_ico图标
			json.width = 120;
			json.height = 32;
			this.$nodeDom[id] = $("<div class='GooFlow_item item_ellipse" + mark + "' id='" + id + "' style='top:" + json.top + "px;left:" + json.left + "px' nodeInfo=" + json.nodeInfo + "><table cellspacing='0'><tr><td class='ico'><i class='offset_ico_" + json.type + "'></i></td></tr></table><div  style='display:none'><div class='rs_close'></div></div><div class='span'>" + json.name + "</div></div>");
		} else { //图标默认是矩形
			if(!json.width || json.width < 100) json.width = 120;
			if(!json.height || json.height < 24) json.height = 32;
			if(!json.top || json.top < 0) json.top = 0;
	...
	initWorkForNode: function() {
		...
		this.$workArea.delegate(".ico", "mousedown", {
			...
			if(e.button == 2) return false;
			var This = e.data.inthis;
			if(This.$nowType == "direct") return; //直线不可以移动
		...
		this.$workArea.delegate(".GooFlow_item > .span", "dblclick", {
			...
			//			var t = getElCoordinate(This.$workArea[0]);
			//			var nodeClassName = $(this.parentNode).find('i').attr('class');
			//			This.$textArea.val(oldTxt).css({
			//				display: "block",
			//				height: $(this).height(),
			//				width: 100,
			//				left: t.left + This.$nodeData[id].left - This.$workArea[0].parentNode.scrollLeft - 273,
			//				top: t.top + This.$nodeData[id].top - This.$workArea[0].parentNode.scrollTop + 0
			//			}).data("id", This.$focus).focus();
			//			This.$workArea.parent().one("mousedown", function(e) {
			//				if(e.button == 2) return false;
			...
			var oldTxt = this.innerHTML;
			var nodeClassName = $(this.parentNode).find('i').attr('class');
			nodeType = getNodeType(nodeClassName);
			var This = e.data.inthis;
			currentEditNodeID = $(this).parents(".GooFlow_item").attr("id");
			var t = getElCoordinate(This.$workArea[0]);

			$("#container").animate({
				width: "85%"
			});
			$("#myPropertyForm").animate({
				width: "15%"
			});
			$("#split").animate({
				right: "13.3%"
			...
			$("#split").css("background", "#FFC125");
			//var attachedPros = $("#databasePros");
			removeFormerPros();
			var attachedPros = getAttachedPros(nodeClassName);
			$("#nodeSpecificInfo").append(attachedPros);
			attachedPros.css("display", "block");
			$("#nodeGeneralInfo").find('input').eq(0).val(currentEditNodeID);
			$("#nodeGeneralInfo").find('input').eq(1).val(oldTxt);
			$("#nodeGeneralInfo").find('input').eq(2).val(nodeType);
			$("#propertyForm").show('slow');
			isShowPropertyForm = true;
			return;
		});
	...
	loadData: function(data) {
		...
		for(var i in data.nodes) {
			this.addNode(i, data.nodes[i], true);
			var nodeId = i.toString();
			var nodeName = data.nodes[i].name.toString();
			$('#' + nodeId).find('td').eq(1).text(nodeName);
			var a = $("#" + nodeId).find('td').eq(1).html();
			console.log(a);
		}
	...
	clearData: function() {
		...
		this.$deletedItem = {};
		//把各类型节点数清零
		isStartNodeExisted = false; //起始节点只能有一个
		endCnt = 0;
		existedWorkCnt = 0;
		execSQLCnt = 0;
		variableSetCnt = 0;
...
function getNodeType(node) {
	if(node.indexOf("start") > 0)
		...
		return "终止节点";
	} else if(node.indexOf("database") > 0) {
		return "数据库节点";
	} else if(node.indexOf("dbtable") > 0) {
		return "数据表节点";
	} else if(node.indexOf("_function") > 0) {
		return "函数节点";
	} else if(node.indexOf("event") > 0) {
		return "事件节点";
	} else if(node.indexOf("query") > 0) {
		return "查询节点";
	} else if(node.indexOf("report") > 0) {
		return "报表节点";
	} else if(node.indexOf("backup") > 0) {
		return "备份节点";
	} else if(node.indexOf("field") > 0) {
		return "字段节点";
	} else if(node.indexOf("operate") > 0) {
		return "操作节点";
	}
}
//获取附属属性节点
function getAttachedPros(node) {
	if(node.indexOf("start") > 0)
		return $("#startPros");
	else if(node.indexOf("end") > 0) {
		return $("#endPros");
...
//删除属性表中上一个节点的属性
function removeFormerPros() {
	var nodeInfoPros = $("#propertyForm").find('form');
	if(nodeInfoPros.length > 0) {
		$("#propertyForm").find('div').eq(3).remove();
		console.log("已删除");
	}
}